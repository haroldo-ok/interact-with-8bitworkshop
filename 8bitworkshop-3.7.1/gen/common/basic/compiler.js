...
var BASICParser = /** @class */ (function () {
    ...
    BASICParser.prototype.expectTokens = function (strlist, msg) {
        ...
        if (strlist.indexOf(tokstr) < 0) {
            this.compileError(msg || "There should be a " + strlist.map(function (s) { return "\"" + s + "\""; }).join(' or ') + " here.");
        }
        return tok;
    };
    ...
    BASICParser.prototype._tokenize = function (line) {
        ...
        var splitre = this.opts.optionalWhitespace && new RegExp('(' + this.opts.validKeywords.map(function (s) { return "" + s; }).join('|') + ')');
                        ...
                        if (splittoks.length > 1) {
                            splittoks.forEach(function (ss) {
                                ...
                                if (/^[0-9]+$/.test(ss))
                                    ...
                                    _this.compileError("Try adding whitespace before \"" + ss + "\".");
    ...
    BASICParser.prototype.stmt__DIM = function () {
        ...
        lexprs.forEach(function (arr) {
            if (arr.args == null || arr.args.length == 0)
                ...
                _this.dialectErrorNoSupport("arrays with more than " + _this.opts.maxDimensions + " dimensionals");
            for (var _i = 0, _a = arr.args; _i < _a.length; _i++) {
                ...
                if (isLiteral(arrdim) && arrdim.value < _this.opts.defaultArrayBase)
                    _this.compileError("An array dimension cannot be less than " + _this.opts.defaultArrayBase + ".", arrdim.$loc);
            }
        });
