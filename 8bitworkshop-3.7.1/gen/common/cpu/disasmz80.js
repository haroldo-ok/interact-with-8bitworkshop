...
function disassembleZ80(pc, b0, b1, b2, b3) {
    ...
    if (!am || !am.length)
        am = "??";
    if (/\bxx\b/.test(am)) {
        am = am.replace(/\bxx\b/, '$' + util_1.hex(bytes[n] + (bytes[n + 1] << 8), 4));
        n += 2;
        isaddr = true;
    }
        ...
        if (am.startsWith('j')) {
            ...
            offset &= 0xffff;
            am = am.replace(/\bx\b/, '$' + util_1.hex(offset, 4));
            isaddr = true;
        }
        else {
            am = am.replace(/\bx\b/, '$' + util_1.hex(bytes[n], 2));
        }
        n += 1;
    }
