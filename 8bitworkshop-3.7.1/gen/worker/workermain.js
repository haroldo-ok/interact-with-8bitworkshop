...
var emglobal = this['window'] || this['global'] || this;
...
function getWorkFileAsString(path) {
    return workfs[path] && workfs[path].data; // TODO
}
...
function parseListing(code, lineMatch, iline, ioffset, iinsns, icycles) {
    var lines = [];
    var lineofs = 0;
    code.split(re_crlf).forEach(function (line, lineindex) {
...
function linkLD65(step) {
    ...
    if (staleFiles(step, [binpath])) {
        ...
        for (var ident in symbolmap) {
            ...
            if (m) {
                var seg = m[1];
                var segstart = symbolmap['__' + seg + '_RUN__'] || symbolmap['__' + seg + '_START__'];
                var segsize = symbolmap['__' + seg + '_SIZE__'];
                var seglast = symbolmap['__' + seg + '_LAST__'];
...
function setupRequireFunction() {
    ...
    emglobal['require'] = function (modname) {
...
function assembleNESASM(step) {
    ...
    putWorkFile(sympath, asym);
    if (alst)
        putWorkFile(lstpath, alst); // listing optional (use LIST)
    // return unchanged if no files changed
...
function parseXMLPoorly(s) {
    var re = /[<]([/]?)([?a-z_-]+)([^>]*)[>]+|(\s*[^<]+)/gi;
    var m;
    //var i=0;
    var stack = [];
    while (m = re.exec(s)) {
        var _m0 = m[0], close = m[1], ident = m[2], attrs = m[3], content = m[4];
        //if (i++<100) console.log(close,ident,attrs,content);
        if (close) {
            var top = stack.pop();
            if (top.type != ident)
                throw "mismatch close tag: " + ident;
            ...
            stack[stack.length - 1].text = content.trim();
        }
    }
    return top;
}
