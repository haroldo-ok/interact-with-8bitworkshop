...
function translateVerilatorOutputToJS(htext, cpptext) {
    ...
    function getStats(o) {
        var nmembits = 0;
        var nlines = 0;
        for (var _i = 0, _a = o.signals.concat(o.ports); _i < _a.length; _i++) {
            var sig = _a[_i];
            var len = sig.len || 0;
            if (sig.arrdim)
                for (var _b = 0, _c = sig.arrdim; _b < _c.length; _b++) {
                    var n = _c[_b];
                    len *= n;
                }
            nmembits += len;
        }
        for (var _d = 0, _e = o.funcs; _d < _e.length; _d++) {
            var fn = _e[_d];
            nlines += fn.split('\n').length;
        }
...
//# sourceMappingURL=verilator2js.js.map