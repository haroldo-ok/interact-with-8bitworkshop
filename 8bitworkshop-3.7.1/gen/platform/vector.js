...
var ASTEROIDS_KEYCODE_MAP = emu_1.makeKeycodeMap([
    [emu_1.Keys.B, 3, 0xff],
    [emu_1.Keys.A, 4, 0xff],
...
var GRAVITAR_KEYCODE_MAP = emu_1.makeKeycodeMap([
    [emu_1.Keys.B, 1, -0x1],
    [emu_1.Keys.A, 1, -0x2],
...
var AtariVectorPlatform = function (mainElement) {
    ...
    this.start = function () {
        ...
        audio = audio_1.newPOKEYAudio(2);
        video.create();
        timer = new emu_1.AnimationTimer(60, this.nextFrame.bind(this));
        emu_1.setKeyboardFromMap(video, switches, ASTEROIDS_KEYCODE_MAP);
    };
    this.advance = function (novideo) {
        if (!novideo)
            video.clear();
        var debugCond = _this.getDebugCallback();
        clock = 0;
...
var AtariColorVectorPlatform = function (mainElement) {
    ...
    this.start = function () {
        ...
        video.create();
        timer = new emu_1.AnimationTimer(60, this.nextFrame.bind(this));
        emu_1.setKeyboardFromMap(video, switches, GRAVITAR_KEYCODE_MAP);
    };
    this.advance = function (novideo) {
        if (!novideo)
            video.clear();
    ...
    this.loadROM = function (title, data) {
        ...
        vecrom = emu_1.padBytes(data.slice(0x7000), 0x3800);
        this.reset();
    };
    this.isRunning = function () {
        return timer && timer.isRunning();
