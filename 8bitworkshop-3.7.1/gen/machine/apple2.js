...
var LOAD_BASE = VM_BASE;
var PGM_BASE = VM_BASE;
var HDR_SIZE = PGM_BASE - LOAD_BASE;
var AppleII = /** @class */ (function (_super) {
    __extends(AppleII, _super);
    function AppleII() {
        var _this = _super.call(this) || this;
        // approx: http://www.cs.columbia.edu/~sedwards/apple2fpga/
        _this.cpuFrequency = 1022727;
        _this.sampleRate = _this.cpuFrequency;
        _this.cpuCyclesPerLine = 65;
        _this.cpuCyclesPerFrame = _this.cpuCyclesPerLine * 262;
        _this.canvasWidth = 280;
        _this.numVisibleScanlines = 192;
        _this.numTotalScanlines = 262;
        _this.defaultROMSize = 0xbf00 - 0x803; // TODO
        _this.ram = new Uint8Array(0x13000); // 64K + 16K LC RAM - 4K hardware + 12K ROM
        _this.cpu = new MOS6502_1.MOS6502();
        _this.grdirty = new Array(0xc000 >> 7);
        _this.grparams = { dirty: _this.grdirty, grswitch: GR_TXMODE, mem: _this.ram };
        _this.kbdlatch = 0;
        _this.soundstate = 0;
        // language card switches
        _this.auxRAMselected = false;
        _this.auxRAMbank = 1;
        _this.writeinhibit = true;
        // value to add when reading & writing each of these banks
        // bank 1 is E000-FFFF, bank 2 is D000-DFFF
        _this.bank2rdoffset = 0;
        _this.bank2wroffset = 0;
        // disk II
        _this.slots = new Array(8);
        ...
        _this.fakeDrive = {
            readROM: function (a) {
                switch (a) {
                    ...
                    case 0: {
                        ...
                        if (_this.rom)
                            _this.ram.set(_this.rom.slice(HDR_SIZE), PGM_BASE);
                        return 0x4c;
                    }
            ...
            readConst: function (a) {
                return 0;
            },
            read: function (a) { return _this.floatbus(); },
            write: function (a, v) { }
        };
        _this.bios = new util_1.lzgmini().decode(util_1.stringToByteArray(atob(APPLEIIGO_LZG)));
        _this.bios[0x39a] = 0x60; // $d39a = RTS
        _this.ram.set(_this.bios, 0xd000);
        _this.ram[0xbf00] = 0x4c; // fake DOS detect for C
        _this.ram[0xbf6f] = 0x01; // fake DOS detect for C
        _this.connectCPUMemoryBus(_this);
        return _this;
    }
    AppleII.prototype.saveState = function () {
        // TODO: automagic
        return {
            ...
            slots: this.slots.map(function (slot) { return slot && slot['saveState'] && slot['saveState'](); }),
            inputs: null
        };
    };
    AppleII.prototype.loadState = function (s) {
    ...
    AppleII.prototype.saveControlsState = function () {
        return { inputs: null, kbdlatch: this.kbdlatch };
    };
    AppleII.prototype.loadControlsState = function (s) {
        this.kbdlatch = s.kbdlatch;
    };
    AppleII.prototype.loadROM = function (data) {
    ...
    AppleII.prototype.reset = function () {
    ...
    AppleII.prototype.skipboot = function () {
    ...
    AppleII.prototype.readConst = function (address) {
        if (address < 0xc000) {
    ...
    AppleII.prototype.read = function (address) {
        address &= 0xffff;
    ...
    AppleII.prototype.write = function (address, val) {
        address &= 0xffff;
        val &= 0xff;
        if (address < 0xc000) {
    ...
    AppleII.prototype.floatbus = function () {
    ...
    AppleII.prototype.connectVideo = function (pixels) {
        _super.prototype.connectVideo.call(this, pixels);
        this.ap2disp = this.pixels && new Apple2Display(this.pixels, this.grparams);
    };
    AppleII.prototype.startScanline = function () {
    };
    AppleII.prototype.drawScanline = function () {
        // TODO: draw scanline via ap2disp
    };
    AppleII.prototype.advanceFrame = function (trap) {
        var clocks = _super.prototype.advanceFrame.call(this, trap);
        this.ap2disp && this.ap2disp.updateScreen();
        return clocks;
    };
    AppleII.prototype.advanceCPU = function () {
        this.audio.feedSample(this.soundstate, 1);
        return _super.prototype.advanceCPU.call(this);
    };
    AppleII.prototype.setKeyInput = function (key, code, flags) {
    ...
    AppleII.prototype.doLanguageCardIO = function (address) {
        switch (address & 0x0f) {
    ...
    AppleII.prototype.setupLanguageCardConstants = function () {
    ...
    AppleII.prototype.getDebugCategories = function () {
        return ['CPU', 'Stack', 'I/O', 'Disk'];
    };
    AppleII.prototype.getDebugInfo = function (category, state) {
        switch (category) {
            case 'I/O': return "AUX RAM Bank:   " + state.auxRAMbank +
                "\nAUX RAM Select: " + state.auxRAMselected +
                "\nAUX RAM Write:  " + !state.writeinhibit +
                "\n\nGR Switches: " + util_1.printFlags(state.grswitch, ["Graphics", "Mixed", "Page2", "Hires"], false) +
                "\n";
...
var Apple2Display = function (pixels, apple) {
    ...
    var loresColor = [
        util_1.RGBA(0, 0, 0),
        util_1.RGBA(227, 30, 96),
        util_1.RGBA(96, 78, 189),
        util_1.RGBA(255, 68, 253),
        util_1.RGBA(0, 163, 96),
        util_1.RGBA(156, 156, 156),
        util_1.RGBA(20, 207, 253),
        util_1.RGBA(208, 195, 255),
        util_1.RGBA(96, 114, 3),
        util_1.RGBA(255, 106, 60),
        util_1.RGBA(156, 156, 156),
        util_1.RGBA(255, 160, 208),
        util_1.RGBA(20, 245, 60),
        util_1.RGBA(208, 221, 141),
        util_1.RGBA(114, 255, 208),
...
var DiskIIState = /** @class */ (function () {
    function DiskIIState() {
        this.track = 0;
        this.read_mode = true;
...
var DiskII = /** @class */ (function (_super) {
    __extends(DiskII, _super);
    function DiskII(emu, image) {
        var _this = _super.call(this) || this;
        _this.emu = emu;
        _this.data = new Array(NUM_TRACKS);
        for (var i = 0; i < NUM_TRACKS; i++) {
            var ofs = i * 16 * 256;
            _this.data[i] = nibblizeTrack(254, i, image.slice(ofs, ofs + 16 * 256));
        }
        return _this;
    }
    DiskII.prototype.saveState = function () {
        var s = {
    ...
    DiskII.prototype.loadState = function (s) {
        for (var i = 0; i < NUM_TRACKS; i++)
    ...
    DiskII.prototype.toLongString = function () {
        return "Track:  " + (this.track / 2) +
            "\nOffset: " + (this.track_index) +
            "\nMode:   " + (this.read_mode ? "READ" : "WRITE") +
            "\nMotor:  " + this.motor +
            "\nData:   " + (this.track_data ? util_1.hex(this.track_data[this.track_index]) : '-') +
            "\n";
    };
    DiskII.prototype.read_latch = function () {
    ...
    DiskII.prototype.write_latch = function (value) {
        this.track_index = (this.track_index + 1) % TRACK_SIZE;
        if (this.track_data != null)
            this.track_data[this.track_index] = value;
    };
    DiskII.prototype.readROM = function (address) { return DISKII_PROM[address]; };
    DiskII.prototype.readConst = function (address) { return DISKII_PROM[address]; };
    DiskII.prototype.read = function (address) { return this.doIO(address, 0); };
    DiskII.prototype.write = function (address, value) { this.doIO(address, value); };
    DiskII.prototype.doIO = function (address, value) {
        switch (address & 0x0f) {
            /*
