...
var ASTROCADE_KEYCODE_MAP = emu_1.makeKeycodeMap([
    // player 1
...
var _BallyAstrocade = function (arcade) {
    var _this = this;
    var cpu;
    var ram;
    var membus, iobus;
    ...
    var swidth = arcade ? 320 : 160;
    var sheight = arcade ? 204 : 102;
    var swbytes = swidth >> 2;
    ...
    this.drawScanline = function (sl) {
        // interrupt
        if (sl == inlin && (inmod & 0x8)) {
            cpu.retryInterrupts = !(inmod & 0x4);
            _this.probe && _this.probe.logInterrupt(infbk);
            cpu.interrupt(infbk);
        }
    ...
    this.init = function (machine, c, r, inp, psgg) {
        cpu = c;
        ram = r;
        inputs = inp;
        psg = psgg;
        //bios = padBytes(ASTROCADE_MINIMAL_BIOS, 0x2000);
        bios = emu_1.padBytes(new util_1.lzgmini().decode(util_1.stringToByteArray(atob(ASTROLIBRE_BIOS_LZG))), 0x2000);
        if (!arcade) {
            // game console
            membus = {
            ...
            membus = {
                read: emu_1.newAddressDecoder([
                    ...
                    [0x0000, 0xafff, 0xffff, function (a) { return rom ? rom[a] : 0; }],
                ]),
                write: emu_1.newAddressDecoder([
            ...
            write: function (addr, val) {
                ...
                switch (addr) {
                    ...
                    case 0x19: // XPAND
                        ...
                        break;
                    default:
                        console.log('IO write', util_1.hex(addr, 4), util_1.hex(val, 2));
                        break;
                }
            }
        };
    ...
    this.toLongString = function (st) {
        var s = "";
        //s += " Scan Y: " + st.sl;
        s += "\n  INLIN: " + st.inlin;
        s += "\n  VERBL: " + st.verbl;
        s += "\nMAGICOP: $" + util_1.hex(st.magicop);
        s += "\n  XPAND: $" + util_1.hex(st.xpand);
        s += "\nXPLOWER: " + st.xplower;
        s += "\n SHIFT2: $" + util_1.hex(st.shift2);
        s += "\n  HORCB: $" + util_1.hex(st.horcb);
        s += "\n  INMOD: $" + util_1.hex(st.inmod);
        s += "\n  INFBK: $" + util_1.hex(st.infbk);
        s += "\n  INTST: $" + util_1.hex(st.intst); // intercept status
        s += "\nPalette: ";
        for (var i = 0; i < 8; i++)
            s += util_1.hex(palinds[i]);
        s += "\n";
        return s;
    };
...
var BallyAstrocade = /** @class */ (function (_super) {
    __extends(BallyAstrocade, _super);
    function BallyAstrocade(arcade) {
        var _this = _super.call(this) || this;
        _this.cpuFrequency = 1789000;
        _this.numTotalScanlines = 262;
        _this.sampleRate = 60 * 262 * audioOversample;
        _this.cpu = new ZilogZ80_1.Z80();
        _this.psg = new AstrocadeAudio(new audio_1.MasterAudio());
        _this.audioadapter = new audio_1.TssChannelAdapter(_this.psg.psg, audioOversample, _this.sampleRate);
        _this.handler = emu_1.newKeyboardHandler(_this.inputs, ASTROCADE_KEYCODE_MAP);
        _this.defaultROMSize = arcade ? 0xb000 : 0x2000;
        _this.ram = new Uint8Array(arcade ? 0x5000 : 0x1000);
        _this.numVisibleScanlines = arcade ? 204 : 102;
        _this.canvasWidth = arcade ? 320 : 160;
        _this.cpuCyclesPerLine = (_this.cpuFrequency / (60 * _this.numVisibleScanlines * 1.286)) | 0;
        ...
        _this.m = new _BallyAstrocade(arcade);
        _this.m.init(_this, _this.cpu, _this.ram, _this.inputs, _this.psg);
        return _this;
    }
    BallyAstrocade.prototype.read = function (a) {
        return this.m.membus.read(a);
    };
    BallyAstrocade.prototype.write = function (a, v) {
        this.m.membus.write(a, v);
    };
    BallyAstrocade.prototype.connectVideo = function (pixels) {
    ...
    BallyAstrocade.prototype.preFrame = function () {
        this.m.resetWaitStates(0);
    };
    BallyAstrocade.prototype.postFrame = function () {
    ...
    BallyAstrocade.prototype.setPaddleInput = function (controller, value) {
        switch (controller) {
            case 0:
    ...
    BallyAstrocade.prototype.startScanline = function () {
        this.audio && this.audioadapter.generate(this.audio);
    };
    BallyAstrocade.prototype.drawScanline = function () {
    ...
    BallyAstrocade.prototype.advanceCPU = function () {
    ...
    BallyAstrocade.prototype.loadROM = function (data) {
        _super.prototype.loadROM.call(this, data);
        this.m.loadROM(this.rom);
        this.reset();
    };
    BallyAstrocade.prototype.loadBIOS = function (data) {
        this.m.loadBIOS(emu_1.padBytes(data, 0x2000));
        this.reset();
    };
    BallyAstrocade.prototype.reset = function () {
        _super.prototype.reset.call(this);
        this.m.reset();
    };
    BallyAstrocade.prototype.loadState = function (state) {
        this.m.loadState(state);
    };
    BallyAstrocade.prototype.saveState = function () {
        return this.m.saveState();
    };
    BallyAstrocade.prototype.getDebugCategories = function () {
        return ['CPU', 'Stack', 'Astro'];
    };
    BallyAstrocade.prototype.getDebugInfo = function (category, state) {
        switch (category) {
            case 'Astro': return this.m.toLongString(state);
        }
    };
    return BallyAstrocade;
...
var AstrocadeAudio = /** @class */ (function (_super) {
    __extends(AstrocadeAudio, _super);
    function AstrocadeAudio() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    AstrocadeAudio.prototype.setACRegister = function (addr, val) {
        addr &= 0x7;
        val &= 0xff;
        switch (addr) {
            case 0:
    ...
    return AstrocadeAudio;
}(audio_1.AY38910_Audio));
exports._BallyArcade = function () {
...
for (var i = 0; i < 256; i++) {
    ...
    ASTROCADE_PALETTE[i] = util_1.rgb2bgr(x) | 0xff000000;
}
/*
